...
SELECT id, 'news', slug, published_slug, title, excerpt, category, tags_json, cover_media_id, status, draft_revision_id, published_revision_id, published_at, created_at, updated_at, created_by
FROM news_posts;

INSERT OR IGNORE INTO content_revisions (id, post_id, version, content_json, created_at, created_by, published_at)
SELECT id, post_id, version, content_json, created_at, created_by, published_at
FROM news_revisions;
