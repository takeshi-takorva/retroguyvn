...
const field=(label,node)=>{const wrap=document.createElement('label');wrap.className='field';const title=document.createElement('span');title.textContent=label;wrap.append(title,node);return wrap;};
...
const textarea=(value,onInput)=>{const node=document.createElement('textarea');node.value=value??'';node.addEventListener('input',()=>onInput(node.value));return node;};
...
export function renderBlockCard(block,index,media,actions){
  ...
  if(block.type==='text')body.append(field('Text',textarea(block.text,v=>set('text',v))));
  ...
  }else if(block.type==='quote')body.append(field('Quote',textarea(block.text,v=>set('text',v))),field('Attribution',input(block.attribution,v=>set('attribution',v))));
