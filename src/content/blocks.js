import { cleanMediaId, cleanText, enforceWordLimit, contentError } from './model.js';

function safeUrl(value) {
  const url = cleanText(value, 2048);
  if (!url) throw contentError('Link URL is required');
  if (url.startsWith('/')) return url;
  let parsed;
  try { parsed = new URL(url); } catch { throw contentError('Link must use a safe http(s) URL'); }
  if (!['http:', 'https:'].includes(parsed.protocol)) throw contentError('Link must use a safe http(s) URL');
...
export function normalizePostBlock(block = {}) {
  ...
  throw contentError(`Unsupported block type: ${type || 'empty'}`);
}
