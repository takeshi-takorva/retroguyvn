const SCHEMA = [
  ...
  `CREATE TABLE IF NOT EXISTS ota_devices (device_id TEXT PRIMARY KEY, hardware_code TEXT NOT NULL, boot_version TEXT, game_version TEXT, channel TEXT NOT NULL DEFAULT 'stable' CHECK(channel IN ('stable','beta','dev')), first_seen_at TEXT NOT NULL, last_seen_at TEXT NOT NULL, last_check_at TEXT, last_download_at TEXT, last_release_id TEXT, last_release_offered_at TEXT, last_ip TEXT, last_user_agent TEXT)`,
...
async function ensureRuntimeUpgrades(db) {
  ...
  if (!columns.has('last_release_offered_at')) {
    await db.prepare('ALTER TABLE ota_devices ADD COLUMN last_release_offered_at TEXT').run();
  }
}
